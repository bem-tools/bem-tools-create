                    ...
                    return Promise.all(currentLevels.map(function(relLevel) {
                        ...
                        return config.level(level).then(function(levelOptions) {
                            ...
                            if (isFileGlob) {
                                var file = path.basename(filepathOrInput),
                                    ...
                                    naming = typeof levelOptions.naming === 'string' ?
                                        ...
                                        levelOptions.naming || bemNamingPresets.origin;
                            ...
                            return Promise.all(currentTechs.map(function(tech) {
                                // fixme here!!!
                                var schemeOptions = Object.assign({}, bemNamingPresets.origin,
                                    levelOptions, levelOptions.naming,
                                    levelOptions.scheme ? { fs: { scheme: levelOptions.scheme } } : {},
                                    levelOptions.schemeOptions);

                                // FIXME: hardcode!
                                schemeOptions.fs.pattern = '${layer}/${entity}.${tech}';
                                var pathToFile = bemNamingCellStringify(schemeOptions)(
                                        new BemCell({
                                            entity: entity,
                                            tech: tech,
